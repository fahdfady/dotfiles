                    ...
                    MouseArea {
                        ...
                        onReleased: {
                            ...
                            if (targetWorkspace !== -1 && targetWorkspace !== windowData?.workspace.id) {
                                ...
                                const targetX = Math.round((monitorData?.x ?? 0) + (percentageX / 100) * (monitorData?.width ?? 0) / monitorScale)
                                const targetY = Math.round((monitorData?.y ?? 0) + (percentageY / 100) * (monitorData?.height ?? 0) / monitorScale)
                                Hyprland.dispatch(`hl.dsp.window.move({ x = ${targetX}, y = ${targetY}, relative = false, window = "address:${window.windowData?.address}" })`)
                            }
                        }
                        onClicked: (event) => {
